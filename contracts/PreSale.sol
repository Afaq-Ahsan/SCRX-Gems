...
contract PreSale is Rounds, ReentrancyGuardTransient {
    ...
    /// @notice Allows purchasing tokens using claimable amounts, with price validation and fee distribution.
    /// @param token The ERC20 token (or ETH) used for purchase.
    /// @param referenceTokenPrice The reference price of the token (from an oracle or other source).
    /// @param referenceNormalizationFactor The normalization factor for the reference price.
    /// @param amount The amount of tokens (or ETH) being spent.
    /// @param minAmountToken The minimum acceptable amount of tokens to be received after calculation.
    /// @param indexes Array of indexes for price lookup in rounds.
    /// @param recipient The address receiving the purchased tokens.
    /// @param round The specific sale round identifier.
